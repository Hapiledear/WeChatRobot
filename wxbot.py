    ...
    def request(self, method, url, params=None, data=None, headers=None, cookies=None, files=None, auth=None,
        ...
        for i in range(3):
            try:
                ...
                LOGGER.exception(e.message, traceback.format_exc())
                continue
...
class WXBot:
    ...
    def get_contact(self):
        ...
        while int(dic["Seq"]) != 0:
            LOGGER.info("Geting contacts. Get %s contacts for now" % dic["MemberCount"])
    ...
    def get_big_contact(self):
        ...
        while self.cursor < total_len:
            ...
            LOGGER.info(" Get batch contacts")
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            if content.find('http://weixin.qq.com/cgi-bin/redirectforward?args=') != -1:
                ...
                if self.DEBUG:
                    try:
                        ...
                        LOGGER.exception('    %s[Text] (illegal text).' % msg_prefix)
        elif mtype == 3:
            msg_content['type'] = 3
            ...
            if self.DEBUG:
                LOGGER.debug('    %s[useradd] %s' % (msg_prefix,msg['RecommendInfo']['NickName']))
        elif mtype == 42:
            msg_content['type'] = 5
            ...
            if self.DEBUG:
                ...
                LOGGER.debug('    | content: %s' % (msg.get('content')[:20] if msg.get('content') else "unknown"))
                LOGGER.debug('    --------------------------')
            ...
            if self.DEBUG:
                LOGGER.debug('    %s[video] %s' % (msg_prefix, msg_content['data']))
        else:
            msg_content['type'] = 99
            msg_content['data'] = content
            if self.DEBUG:
        ...
        for msg in r['AddMsgList']:
            ...
            if msg['MsgType'] == 51 and msg['StatusNotifyCode'] == 4:  # init message
                ...
                if self.is_big_contact and len(self.full_user_name_list) == 0:
                    ...
                    LOGGER.info("[INFO] Contact list is too big. Now start to fetch member list .")
                ...
                # LOGGER u'[Friend Request]'
                # LOGGER u'       Nickname：' + msg['RecommendInfo']['NickName']
                # LOGGER u'       附加消息：'+msg['RecommendInfo']['Content']
                # # LOGGER u'Ticket：'+msg['RecommendInfo']['Ticket'] # Ticket添加好友时要用
                # LOGGER u'       微信号：'+username #未设置微信号的 腾讯会自动生成一段微信ID 但是无法通过搜索 搜索到此人
            ...
            if self.DEBUG and msg_type_id != 0:
                LOGGER.debug( u'[MSG] %s:' % user['name'])
    ...
    def proc_msg(self):
        ...
        while True:
            ...
            try:
                ...
                # LOGGER '[DEBUG] sync_check:', retcode, selector
                ...
                LOGGER.exception('[ERROR] Except in proc_msg')
    ...
    def add_friend_to_group(self,uid,group_name):
        ...
        LOGGER.debug('[DEBUG] group_name:%s group_num:%s' % (group_name,group_num))
        #通过群id判断uid是否在群中
    ...
    def upload_media(self, fpath, is_img=False):
        ...
        try:
            ...
            if json.loads(r.text)['BaseResponse']['Ret'] != 0:
                LOGGER.error('[ERROR] Upload media failure.')
                return None
    ...
    def send_msg(self, name, word, isfile=False):
        ...
        if uid is not None:
            ...
            if self.DEBUG:
                LOGGER.error('[ERROR] This user does not exist .')
            return True
    ...
    def run(self):
        try:
            ...
            LOGGER.exception('[ERROR] Web WeChat run failed --> %s' % e)
            self.status = 'loginout'
    ...
    def gen_qr_code(self, qr_file_path):
        ...
        if self.conf['qr'] == 'png':
            ...
            LOGGER.info(qr.terminal(quiet_zone=1))
